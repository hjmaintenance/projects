...
namespace MIT.UI.Main.MainFrame {
  public class MainFrameFinalBase : CommonUIComponentBase {
    [Inject]
    ...
    protected async void OnOpenPage(MainFrameData mainFrameData) {
          ...
          Console.WriteLine($"datas :{mainFrameData}");
      ...
      if (index < 0) {
        ...
        try {
          ...
          {
          Console.WriteLine("datatable 널이얌");
            return;
          }
        ...
        */
        ...
        var page = new MIT.UI.Main.MainFrame.MainFrameDataButtonData(mainFrameData, new Razor.Pages.Common.MainCommonButtonData() {
          ...
          MENU_ID = mainFrameData.MENU_ID.ToStringTrim(),
        });
      ...
      appData.ActiveTabIndex = index + 1;


      if (index <= 0) {
        appData.ActiveTabMenuID = HomeMenuID;
      }
      else {
        appData.ActiveTabMenuID = Pages[index - 1].MENU_ID.ToStringTrim();
      }

      OnActiveTabIndexChanged(appData.ActiveTabIndex);
      StateHasChanged();
    }
    ...
    protected RenderFragment? BuildPage(MainFrameDataButtonData page, int tabindex) {

      //int CloneIndex = new Interger (tabindex + 1);
      ...
      if (page.PagRF == null) {
        ...
        page.PagRF = renderPage;
      }

      return page.PagRF;
    }
    protected RenderFragment? BuildPage_bk(MainFrameDataButtonData page, int tabindex) {
    ...
    public string HomeMenuID { get; set; } = "MainDashboard";
    ...
    protected RenderFragment? BuildHomePage() {
      ...
      // 캐시에서 어셈블리 가져오거나 새로 로드해서 추가
      var assem = _assemblyCache.GetOrAdd("MIT.UI.UMS", path => Assembly.Load(path));
