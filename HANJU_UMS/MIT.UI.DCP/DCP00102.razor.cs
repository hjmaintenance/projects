...
{
    public class DCP00102Base : CommonUIComponentBase
    {
        protected CommonTextBox? txt_MSG_ID { get; set; }
        protected CommonTextBox? txt_MSG_NAME { get; set; }
        ...
        private async Task<DataTable?> DBSearch()
        {
            ...
            var datatable = await QueryService.ExecuteDatatableAsync("SP_DCP00102_GRD1_SELECT", new Dictionary<string, object?>()
                {
                    { "MSG_ID", txt_MSG_ID?.Text },
                    { "MSG_NAME", txt_MSG_NAME?.Text },
                });
            return datatable;
        }
        ...
        private async Task DBSave(DataTable dataTable)
        {
            ...
            await QueryService.ExecuteNonQuery("SP_DCP00102_GRD1_SAVE", dataTable, new Dictionary<string, object?>()
            {
                { "REG_ID", USER_ID },
            });
        }
        ...
        private async Task DBDelete(DataTable dataTable)
        {
            ...
            await QueryService.ExecuteNonQuery("SP_DCP00102_GRD1_DELETE", dataTable, new Dictionary<string, object?>()
            {
                { "REG_ID", USER_ID },
            });
        }
