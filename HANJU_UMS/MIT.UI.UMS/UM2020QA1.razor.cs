...
* 최종수정 : 25-02-07
* 화면명 : 전기 수용가별 역률 현황
* 프로시저 : P_HMI_USE_YEARELCREV_SELECT01
*/
...
{
    public class UM2020QA1Base : CommonUIComponentBase
    {
        ...
        private void InitControls()
        {
            if (Grd1 == null || Grd1.Grid == null)
                return;
            Grd1.SetCellStyle("COM_NAME", "2_합계,2_합 계");
        ...
        private async Task<DataTable?> DBSearch()
        {
            ...
            var datatable = await QueryService.ExecuteDatatableAsync("P_HMI_USE_YEARELCREV_SELECT01", new Dictionary<string, object?>()
                {
                    {"YYYY", YYYY?.EditValue },
                });
