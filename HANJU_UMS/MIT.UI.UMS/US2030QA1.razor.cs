...
* 최종수정 : 25-02-06
* 화면명 : 전기 월별 사용량 현황
* 프로시저 : P_HMI_USE_YEARELCMONTH_SELECT01
*/
...
{
    public class US2030QA1Base : CommonUIComponentBase
    {
        ...
        private async Task<DataTable?> DBSearch()
        {
            ...
            var datatable = await QueryService.ExecuteDatatableAsync("P_HMI_USE_YEARELCMONTH_SELECT01", new Dictionary<string, object?>()
                {
                    {"YYYY", YYYY?.EditValue },
