...
{
    public class US2110QA1Base : CommonUIComponentBase
    {
        ...
        private void InitControls()
        {
            ...
            // 수용가는 본인 회사 컬럼만 보여주기 위해
            COM_ID = isHanju ? COM_ID : USER_ID;

            if (Grd1 == null || Grd1.Grid == null)
                return;
            Grd1.SetCellStyle("GAUGE_HOUR","합계,합 계");
        ...
        private async Task<DataTable?> DBSearch()
        {
            ...
            var datatable = await QueryService.ExecuteDatatableAsync("P_HMI_USE_DAYSTMTIMETOTAL_SELECT01", new Dictionary<string, object?>()
                {
                    {"DATE", DATE?.EditValue },
