    ...
    public class US2240QA1Base : CommonUIComponentBase
    {
        ...
        private void InitControls()
        {
            ...
            if (Grd1 == null || Grd1.Grid == null)
                return;
            Grd1.SetCellStyle("GAUGE_YEAR", "합계, 합계");
        ...
        private async Task<DataTable?> DBSearch()
        {
            ...
            var datatable = await QueryService.ExecuteDatatableAsync("P_HMI_USE_YEARWATYEAR_SELECT01", new Dictionary<string, object?>()
                {
                    {"SYYYY", SYYYY?.EditValue},
                    {"EYYYY", EYYYY?.EditValue},
                    {"COM_ID", comId.ToStringTrim()},
                    {"GAUGE_TYPE", WATER_TYPE.ToStringTrim()},
                });
