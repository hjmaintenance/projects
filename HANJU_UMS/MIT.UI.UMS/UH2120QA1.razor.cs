...
{
    public class UH2120QA1Base : CommonUIComponentBase
    {
        ...
        private async Task<DataTable?> DBSearch()
        {
            ...
            //string procedureName = "P_HMI_STM_HOUR_SERV_RST";

            //if (!string.IsNullOrEmpty(COM_ID)) //.IsNullOrEmpty("xxxxxxxxxxxxxxx"))
            //{
            //    procedureName = "P_HMI_STM_HOUR_SERV_RST_SELECT02";
            //}

            var datatable = await QueryService.ExecuteDatatableAsync_fix("P_HMI_STM_HOUR_SERV_RST", new Dictionary<string, object?>()
                {
                    {"DATE", DATE?.EditValue },
