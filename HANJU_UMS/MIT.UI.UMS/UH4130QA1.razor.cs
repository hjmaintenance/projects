...
* 작성일자 : 25-01-24
* 최종수정 : 25-01-24
* 프로시저 : P_HMI_FMS_YEAR_WTR_GEN_RST, P_HMI_FMS_YEAR_GEN_RST, P_HMI_FMS_YEAR_BOILER_RSTT
*/
...
{
    public class UH4130QA1Base : CommonUIComponentBase
    {
        ...
        {
            parameters.Add("SYYYY", SYYYY?.EditValue);
            parameters.Add("EYYYY", EYYYY?.EditValue);
        }
        ...
        private async Task<DataTable?> DBSearchWater()
        {
            ...
            var datatable = await QueryService.ExecuteDatatableAsync_fix("P_HMI_FMS_YEAR_WTR_GEN_RST", new Dictionary<string, object?>()
                {
                    {"SYYYY", SYYYY?.EditValue },
                    {"EYYYY", EYYYY?.EditValue }
                }, "P_");
        ...
        private async Task<DataTable?> DBSearchElec()
        {
            ...
            var datatable = await QueryService.ExecuteDatatableAsync_fix("P_HMI_FMS_YEAR_GEN_RST", new Dictionary<string, object?>()
                {
                    {"SYYYY", SYYYY?.EditValue },
                    {"EYYYY", EYYYY?.EditValue }
                }, "P_");
        ...
        private async Task<DataTable?> DBSearchBoiler()
        {
            ...
            var datatable = await QueryService.ExecuteDatatableAsync_fix("P_HMI_FMS_YEAR_BOILER_RSTT", new Dictionary<string, object?>()
                {
                    {"SYYYY", SYYYY?.EditValue },
                    {"EYYYY", EYYYY?.EditValue }
                }, "P_");
