...
* 작성일자 : 25-01-24
* 최종수정 : 25-01-24
* 프로시저 : P_HMI_FMS_MON_WTR_GEN_RST
*/
...
{
    public class UH4120QA1Base : CommonUIComponentBase
    {
        protected IEnumerable<string> ArrayGenType = new[] { "수전", "발전", "보일러" };
        protected string SelectedGenType { get; set; } = "수전";
        protected CommonDateEdit? SYYMM{ get; set; }
        protected CommonDateEdit? EYYMM{ get; set; }
        ...
        private async Task<DataTable?> DBSearchWater()
        {
            ...
            var datatable = await QueryService.ExecuteDatatableAsync_fix("P_HMI_FMS_MON_WTR_GEN_RST", new Dictionary<string, object?>()
                {
                    {"SYYMM", SYYMM?.EditValue ?.Substring(0, 7) },
                    {"EYYMM", EYYMM?.EditValue ?.Substring(0, 7) },
                }, "P_");
        ...
        private async Task<DataTable?> DBSearchElec()
        {
            ...
            var datatable = await QueryService.ExecuteDatatableAsync_fix("P_HMI_FMS_MON_GEN_RST", new Dictionary<string, object?>()
                {
                    {"SYYMM", SYYMM?.EditValue ?.Substring(0, 7) },
                    {"EYYMM", EYYMM?.EditValue ?.Substring(0, 7) },
                }, "P_");
        ...
        private async Task<DataTable?> DBSearchBoiler()
        {
            ...
            var datatable = await QueryService.ExecuteDatatableAsync_fix("P_HMI_FMS_MON_BOILER_RST", new Dictionary<string, object?>()
                {
                    {"SYYMM", SYYMM?.EditValue ?.Substring(0, 7) },
                    {"EYYMM", EYYMM?.EditValue ?.Substring(0, 7) },
                }, "P_");
        ...
        private async Task DBSave(DataTable datatable)
        {
            ...
            await QueryService.ExecuteNonQuery("P_HMI_FMS_MON_WTR_GEN_RST_SAVE", datatable, new Dictionary<string, object?>()
            {
                { "REG_ID", USER_ID },
            });
        }
        ...
        private async Task DBDelete(DataTable datatable)
        {
            ...
            await QueryService.ExecuteNonQuery("P_HMI_FMS_MON_WTR_GEN_RST_DELETE", datatable, new Dictionary<string, object?>()
            {
                { "REG_ID", USER_ID },
            });
        }
