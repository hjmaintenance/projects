...
{
    public class UH4110QA1Base : CommonUIComponentBase
    {
        ...
        {
            parameters.Add("SDATE", SDATE?.EditValue);
            parameters.Add("EDATE", EDATE?.EditValue);
        }
        ...
        private async Task<DataTable?> DBSearchWater()
        {
            ...
            var datatable = await QueryService.ExecuteDatatableAsync_fix("P_HMI_FMS_DAY_WTR_GEN_RST", new Dictionary<string, object?>()
                {
        ...
        private async Task<DataTable?> DBSearchElec()
        {
            ...
            var datatable = await QueryService.ExecuteDatatableAsync_fix("P_HMI_FMS_DAY_GEN_RST", new Dictionary<string, object?>()
                {
                    {"SDATE", SDATE?.EditValue },
                    {"EDATE", EDATE?.EditValue },
                }, "P_");
        ...
        private async Task<DataTable?> DBSearchBoiler()
        {
            ...
            var datatable = await QueryService.ExecuteDatatableAsync_fix("P_HMI_FMS_DAY_BOILER_RST", new Dictionary<string, object?>()
                {
                    {"SDATE", SDATE?.EditValue },
                    {"EDATE", EDATE?.EditValue },
                },"P_");
