...
{
    public class UH2140QA1Base : CommonUIComponentBase
    {
        ...
        private async Task Search()
        {
            ...
            try
            {
                ...
                {
                    SearchedHour = MINUTE?.EditValue ?? SearchedHour;
                }
        ...
        private async Task<DataTable?> DBSearch()
        {
            ...
            //string procedureName = "P_HMI_WATER_MIN_SRV_RST"; // 업체 선택 안한경우

            //if (!string.IsNullOrEmpty(COM_ID)) //.IsNullOrEmpty("xxxxxxxxxxxxxxx"))
            //{
            //    procedureName = "P_HMI_WATER_MIN_SRV_RST_SELECT02"; // 업체 선택한 경우
            //}
            var datatable = await QueryService.ExecuteDatatableAsync_fix("P_HMI_WATER_MIN_SRV_RST", new Dictionary<string, object?>()
                {
                    {"DATE", DATE?.EditValue },
