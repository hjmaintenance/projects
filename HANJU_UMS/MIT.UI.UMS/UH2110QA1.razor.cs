...
public class UH2110QA1Base : CommonUIComponentBase {
  protected CommonDateEdit? DATE { get; set; }
  protected CommonTimeEdit? MINUTE { get; set; }
  protected DateTime SearchedDateTime { get; set; } = DateTime.Now;
  ...
  protected override async Task OnAfterRenderAsync(bool firstRender) {
    ...
    protected async Task InitControls() {
        ...
        await Task.Run(async () => {
            ...
            Grd1.SetCellStyle("MC_NAME", "99_소계", "row2");
            Grd1.SetCellStyle("COM_NAME", "30_합계");
  ...
  protected void OnInputSearchParameter(Dictionary<string, object?> parameters) {
    ...
    parameters.Add("MINUTE", MINUTE?.EditValue);
    ...
    try {
      ...
      var datasource = await DBSearch();
        ...
        if (datasource?.Rows.Count > 0) {
            SearchedDateTime = DateTime.ParseExact(DATE?.EditValue + " " + MINUTE?.EditValue, "yyyy-MM-dd HH", null);
        }
  ...
  private async Task<DataTable?> DBSearch() {
    ...
    //string procedureName = string.IsNullOrWhiteSpace(COM_ID) ? "P_HMI_STM_1MIN_SERV_RST" : "P_HMI_STM_1MIN_SERV_RST_SELECT02"; // 업체 선택 안한경우 : 업체 선택한 경우

    var datatable = await QueryService.ExecuteDatatableAsync_fix("P_HMI_STM_1MIN_SERV_RST", new Dictionary<string, object?>()
        {
                  {"DATE", DATE?.EditValue },
                  {"MINUTE", MINUTE ?.EditValue },
