...
{
    public class UH2080QA1Base : CommonUIComponentBase
    {
        ...
        public CommCode COM_ID{ get; set; }


    protected IEnumerable<CommCode> MC_IDS { get; set; } = new List<CommCode>();
    public string MC_ID {
      get {
        return string.Join(",", MC_IDS?.AsEnumerable().Select(r => r.Name));
      }
    }
    ...
    protected CommonGrid? Grd1 { get; set; }
        ...
        {
            parameters.Add("COM_ID", COM_ID?.Name.ToStringTrim());
        }
        ...
        private async Task<DataTable?> DBSearch()
        {
            ...
            string procedureName = string.IsNullOrWhiteSpace(COM_ID?.Name) ? "P_HMI_ELEC_FACT_FDR_SERV_RST" : "P_HMI_ELEC_FACT_FDR_SERV_COM_RST"; // 업체 선택 안한경우 : 업체 선택한 경우
            ...
            var datatable = await QueryService.ExecuteDatatableAsync_fix(procedureName, new Dictionary<string, object?>()
                {
                    ...
                    {"COM_ID", COM_ID?.Name.ToStringTrim() },
                    {"MC_ID", MC_ID },
                }, "P_");
