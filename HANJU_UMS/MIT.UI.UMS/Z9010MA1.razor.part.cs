  ...
  public partial class Z9010MA1Base : CommonUIComponentBase {
    ...
    private async Task<DataTable?> DBSearch() {
      ...
      var datatable = await QueryService.ExecuteDatatableAsync("P_HMI_COMMUNITY_BOARDLIST_SELECT01", new Dictionary<string, object?>() {
                    {"S_USER_NAME", USER_NAME.ToStringTrim() },
                    {"S_TITLE", BOARD_TITLE.ToStringTrim() },
                });
    ...
    private async Task DBSave(DataTable datatable) {
      ...
      await QueryService.ExecuteNonQuery("P_HMI_COMMUNITY_BOARDINFO01_SAVE", datatable, new Dictionary<string, object?>(){
            });
    }
    ...
    private async Task DBDelete(DataTable datatable) {
      ...
      await QueryService.ExecuteNonQuery("P_HMI_COMMUNITY_BOARDINFO01_DELETE", datatable, new Dictionary<string, object?>()      {
            });
    }
