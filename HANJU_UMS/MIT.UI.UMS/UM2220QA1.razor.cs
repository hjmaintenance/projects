...
{
    public class UM2220QA1Base : CommonUIComponentBase
    {
        ...
        {
            //parameters.Add("YYYY", YYYY?.EditValue);
            //parameters.Add("COM_ID", COM_ID.ToStringTrim());
            //parameters.Add("TP", USE_QTY_TYPE.ToStringTrim());
        }
        ...
        private async Task<DataTable?> DBSearch()
        {
            ...
            var datatable = await QueryService.ExecuteDatatableAsync("P_HMI_USE_MONTHSTMMONTHTOTAL_SELECT01", new Dictionary<string, object?>()
                {
                    {"YYYY", YYYY?.EditValue },
