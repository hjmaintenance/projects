...
function updateColors(type, color) {
    if (type === 'primary') {
        ...
        localStorage.setItem('theme.surface', color.name);
    }
...
function onPresetChange() {
    ...
    localStorage.setItem('theme.preset', preset.value);
    ...
    //$t().preset(presetValue).preset(getPresetExt()).surface(surfacePalette).use({ useDefaultOptions: true });
}

function onMenuModeChange() {
    layoutConfig.menuMode = menuMode.value;
    localStorage.setItem('theme.menuMode', menuMode.value);
}
...
onMounted(() => {
    ...
    console.log('appConfigurator mounted', layoutConfig);
...
<button @click="aaaaa">ccccccccccccccccccccccc</button>
